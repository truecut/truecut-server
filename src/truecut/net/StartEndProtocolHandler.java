...
/**
 * ?�임즈에??FixedLengthProtocolHandler �??�용?��?�? ?�용?��? ?�습?�다.
 *
 * InputListener�??�속받아 ?�신 ?�이??? ?�킷별로 처리?�기 ?�한 ?�래??br>
 * ?�킷 구조???�음�?같음: ?�더(0x1B 0x02) data ?�일(0x1B 0x03)<br>
 ...
 */
public class StartEndProtocolHandler<E> implements InputListener<E> {

	/** ?�킷 ?�더 �??�일 ?�의 : ?�더 = ESC + STX, ?�일 = ESC + ETX */
	public static final byte ESC = 0x1B;
	public static final byte STX = 0x02;
	public static final byte ETX = 0x03;
	
	private byte[] HEADER_TAG = {ESC, STX};
	private byte[] TAIL_TAG = {ESC, ETX}; 
	
	private int startIndex = -1;	// ?�더 발견 ?�치
	 ...
	 * ?�더(2byte) �?��
	 *
	 * @param _tag	?�더, 2byte ?�야 ??
	 * @return	?�더�?2byte�??�닐 경우 false return
	 */
	public boolean setHeaderTag ( byte[] _tag ) {
		if ( _tag.length!=2 )	return false;
	 ...
	 * ?�일(2byte) �?��
	 *
	 * @param _tag	?�일, 2byte ?�야 ??
	 * @return	?�일??2byte�??�닐 경우 false return
	 */
	public boolean setTailTag ( byte[] _tag ) {
		if ( _tag.length!=2 )	return false;
...
// logger.debug ( "message\n" + MessageBuffer.getHexString(buffer, 0, before_pos) );
		...
		while ( true ) {
			...
			current_index = startIndex+2;
			endIndex = MessageBuffer.indexOf ( buffer, current_index, buffer.position(), TAIL_TAG[0], TAIL_TAG[1]  );
			...
			byte[] msg = new byte[endIndex-startIndex-2];
			buffer.position(startIndex+2);
			buffer.get(msg);
			
			handleData ( _channel_or_socket, _message_buffer, msg );
			
			current_index = endIndex+2;
